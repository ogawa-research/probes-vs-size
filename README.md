...
Every statistic in the paper's analysis sections is reproduced by these four
scripts from the JSONs in `data/`. A quick pointer table:
...
| Same-size (8B / 7B) contrast | `b5_results.json` → `same_size_groups` |
...
bounded by gate G2a, see `ledger.md`).
...
verified against this repository once posted.

## Notes on redaction

Informational note strings inside a few measurement/result JSONs were
redacted at release time (an AI-assistant model name and one internal
review-document filename replaced by role labels / neutral references); all
numeric content is byte-identical to the internal originals. The release
build script enforces this mechanically (declared-edit-only diffs plus a
forbidden-token sweep over every file).
